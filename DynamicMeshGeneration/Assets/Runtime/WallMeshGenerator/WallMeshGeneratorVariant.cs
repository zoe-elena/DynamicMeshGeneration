...
[ExecuteAlways]
public class WallMeshGeneratorVariant : MonoBehaviour
{
    ...
    [Space, Tooltip("Right Scale")] public float WidthRight = 1;
    ...
    // One segment is this high on texture scale 1
    [HideInInspector] public float wallHeight = 0.75f;

    [HideInInspector] public float Depth = 1f;

    [HideInInspector] public Vector2 TextureOffset = Vector3.zero;
    ...
    private void SubdivideCubeFront(List<Vector3> _vertices)
    {
        ...
        if (additiveRightExtension > 0)
        {
            ...
            _vertices.Add(new Vector3(Depth, height, WidthRight));
            _vertices.Add(new Vector3(Depth, height, WidthRight - additiveRightExtension));
        }
        ...
        for (int i = 0; i < allExtensions; i++)
        {
            ...
            _vertices.Add(new Vector3(Depth, height, prevUVLength));
            _vertices.Add(new Vector3(Depth, height, prevUVLength - 2));
            prevUVLength -= 2;
        }
        ...
        if (additiveLeftExtension > 0)
        {
            ...
            _vertices.Add(new Vector3(Depth, height, prevUVLength));
            _vertices.Add(new Vector3(Depth, height, prevUVLength - additiveLeftExtension));
    ...
    private void SubdivideCubeBack(List<Vector3> _vertices)
    {
        ...
        if (additiveRightExtension > 0)
        {
            ...
            _vertices.Add(new Vector3(-Depth, height, WidthLeft));
            _vertices.Add(new Vector3(-Depth, height, WidthLeft + additiveRightExtension));
        }
    ...
    private Vector2[] CreateUV(Vector3[] vertices)
    {
        ...
        for (int k = 0; k < columnCount; k++)
        {
            ...
            if (k < columnCount / 2)
            {
                ...
                uv.Add(new Vector2(distToNextVert, height));
    ...
    private void SetTextureVariants(Vector2[] _uv)
    {
        ...
        for (int i = verticesFrontStartIndex; i < _uv.Length - backVerticesCount; i += verticesPerQuad)
        {
            ...
            {
                if (currentSegmentIndex - textureVariantsRight.Count >= textureVariantsLeft.Count)
                    return;
